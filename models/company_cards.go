...
func (o *CompanyCard) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(companyCardAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(companyCardPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert company_cards, could not build conflict column list")
			}

			conflict = make([]string, len(companyCardPrimaryKeyColumns))
			copy(conflict, companyCardPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"company_cards\"", updateOnConflict, ret, update, conflict, insert, opts...)
