...
var (
	tripDBTypes = map[string]string{`ID`: `text`, `Currency`: `text`, `StartDateTime`: `timestamp without time zone`, `EndDateTime`: `timestamp without time zone`, `LegalEntityID`: `text`, `Name`: `text`, `Status`: `text`, `TotalClaim`: `integer`, `UserID`: `text`}
	_           = bytes.MinRead
)
